...
qtl18_effect <- c(0.224, 0.426, 0.611, 0.787, 0.956, 1.12, 1.283, 1.445, 1.1449)
qtl18_sim_percent <- c(5, 10, 15, 20, 25, 30, 35, 40, "recorded")
effect_data_frame <- data.frame(qtl18_effect, qtl18_sim_percent)

sim_percent <- effect_data_frame[effect_data_frame$qtl18_effect == effect,]$qtl18_sim_percent 
...
simulate_data <- function (glmm_model, num_sim) {
	good_simulation <- "no"
		...
		test.mod1 <- try( lod <- get_lod_score(QTL_data$geno18, cbind(simulated_data$sim_1[,1], simulated_data$sim_1[,2]), QTL_data$id ))
		options(warn=1)
		## ... but discard if model does not converge
		if(class(test.mod1) == "try-error") { good_simulation <- "no" } else { good_simulation <- "yes" }
		lod <- get_lod_score(QTL_data$geno18, cbind(simulated_data$sim_1[,1], simulated_data$sim_1[,2]), QTL_data$id )
		effect <- get_effect(QTL_data$geno18, cbind(simulated_data$sim_1[,1], simulated_data$sim_1[,2]), QTL_data$id )
	out_list<-list(lod, effect, good_simulation) 
	return(out_list)
}

#PRODUCE MODEL FOR COURTSHIPS FROM REAL DATA USING lme4:
rm <- glmer(cbind(court_mp, court_cp) ~ geno18  + (1|id),  family = binomial,  data = QTL_data )
#summary(rm)
...
write.csv(sim_run, paste0("../derived_data/beavis_sim_results/QTL_18_sim_results_", as.character(sim_percent), ".csv"),  row.names = FALSE)
