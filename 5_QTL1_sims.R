...
qtl1_effect <- c(0.219, 0.418, 0.601, 0.778, 0.944, 1.11, 1.269, 1.44, 0.9722778)
qtl1_sim_percent <- c(5, 10, 15, 20, 25, 30, 35, 40, "recorded")
effect_data_frame <- data.frame(qtl1_effect, qtl1_sim_percent)

sim_percent <- effect_data_frame[effect_data_frame$qtl1_effect == effect,]$qtl1_sim_percent 
...
simulate_data <- function (glmm_model, num_sim) {
	good_simulation <- "no"
		...
		test.mod1 <- try( lod <- get_lod_score(QTL_data$geno1, cbind(simulated_data$sim_1[,1], simulated_data$sim_1[,2]), QTL_data$id ))
		options(warn=1)
		## ... but discard if model does not converge
		if(class(test.mod1) == "try-error") { good_simulation <- "no" } else { good_simulation <- "yes" }
		lod <- get_lod_score(QTL_data$geno1, cbind(simulated_data$sim_1[,1], simulated_data$sim_1[,2]), QTL_data$id )
		effect <- get_effect(QTL_data$geno1, cbind(simulated_data$sim_1[,1], simulated_data$sim_1[,2]), QTL_data$id )
	out_list<-list(lod, effect, good_simulation) 
	return(out_list)
}

#PRODUCE MODEL FOR COURTSHIPS FROM REAL DATA USING lme4:
rm <- glmer(cbind(court_mp, court_cp) ~ geno1  + (1|id),  family = binomial,  data = QTL_data )
#summary(rm)
...
write.csv(sim_run, paste0("../derived_data/beavis_sim_results/QTL_1_sim_results_", as.character(sim_percent), ".csv"),  row.names = FALSE)